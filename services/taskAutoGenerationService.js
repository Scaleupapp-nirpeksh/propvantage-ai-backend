...
import { notifyTaskAutoGenerated, notifyTaskEscalated } from './notificationService.js';
...
class TaskAutoGenerationService {
  ...
  start() {
    ...
    cron.schedule('0 * * * *', () => {
      console.log('🔄 [TaskAutoGen] Running hourly trigger checks');
      this.runAllChecks();
    });
  ...
  async runAllChecks() {
    if (this.isRunning) {
      console.log('⏳ [TaskAutoGen] Already running, skipping');
      return;
    }
    ...
    try {
      ...
      for (const org of orgs) {
        try {
          ...
          console.error(
            `❌ [TaskAutoGen] Error for org ${org._id}: ${err.message}`
          );
        }
      }
    } catch (err) {
      console.error(`❌ [TaskAutoGen] runAllChecks failed: ${err.message}`);
    } finally {
      this.isRunning = false;
    }
  }
  ...
  async checkOverduePayments(organizationId) {
    try {
      ...
      for (const inst of overdueInstallments) {
        ...
        notifyTaskAutoGenerated({ task: newTask }).catch(() => {});
      }
    } catch (err) {
      console.error(
        `❌ [TaskAutoGen] checkOverduePayments error: ${err.message}`
      );
    }
  }
  ...
  async checkMissedFollowUps(organizationId) {
    try {
      ...
      console.error(
        `❌ [TaskAutoGen] checkMissedFollowUps error: ${err.message}`
      );
    }
  }
  ...
  async checkDelayedMilestones(organizationId) {
    try {
      ...
      console.error(
        `❌ [TaskAutoGen] checkDelayedMilestones error: ${err.message}`
      );
    }
  }
  ...
  async checkNewSaleOnboarding(organizationId) {
    try {
      ...
      console.error(
        `❌ [TaskAutoGen] checkNewSaleOnboarding error: ${err.message}`
      );
    }
  }
  ...
  async generateRecurringTasks() {
    try {
      ...
      console.error(
        `❌ [TaskAutoGen] generateRecurringTasks error: ${err.message}`
      );
    }
  }
  ...
  async checkEscalations() {
    try {
      ...
      for (const task of overdueTasks) {
        ...
        // Notify the manager about the escalation
        notifyTaskEscalated({
          task,
